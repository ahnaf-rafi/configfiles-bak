...
(use-package spell-fu
  ...
  (add-hook 'spell-fu-mode-hook #'aar/spell-init-excluded-faces-h)
  (defvar aar/spell-excluded-faces-alist
    '((markdown-mode
       . (markdown-code-face
...
;;;###autoload
(defun aar/spell-init-excluded-faces-h ()
  "Set `spell-fu-faces-exclude' according to `aar/spell-excluded-faces-alist'."
  (when-let (excluded (cdr (cl-find-if #'derived-mode-p aar/spell-excluded-faces-alist :key #'car)))
    (setq-local spell-fu-faces-exclude excluded)))

;;;###autoload
(defun aar/spell--correct (replace poss word orig-pt start end)
...
;;;###autoload
(defun aar/spell-correct-fn (candidates word)
    ...
    (let ((word (thing-at-point 'word t))
      ...
      (cond
       ...
       (t
        ...
        (setq res (funcall aar/spell-correct-fn (nth 2 poss) word))
        ...
        (cond
         ((stringp res)
          (aar/spell--correct res poss word orig-pt start end))
         ((let ((cmd (car res))
                (wrd (cdr res)))
            (unless (or (eq cmd 'skip)
                        (eq cmd 'break)
                        (eq cmd 'stop))
              (aar/spell--correct cmd poss wrd orig-pt start end)
              (unless (string-equal wrd word)
                (aar/spell--correct wrd poss word orig-pt start end))))))
        (ispell-pdict-save t))))))
#+end_src
